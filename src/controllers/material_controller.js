...
export const extract_material = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({
      data: "No file material or neither PDF nor DOCX file material.",
    });
  } catch (error) {
  ...
  try {
    ...
    if (!classroom_id || !description || !title) {
      return res.status(400).json({
        message:
          "Please provide all fields (classroom_id, description, title).",
      });
    }
...
export const get_all_material_specific_classroom = asyncHandler(
  async (req, res) => {
...
export const update_material = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    updatedMaterial.classroom = classroom_id
      ? classroom_id
      : updatedMaterial.classroom;
    updatedMaterial.description = description
      ? description
      : updatedMaterial.description;
    updatedMaterial.title = title ? title : updatedMaterial.title;
    updatedMaterial.material = req.file
      ? filename_insert
...
export const get_all_materials = asyncHandler(async (req, res) => {
  try {
    const materials = await Material.find().populate("classroom");

    return res.status(200).json({ data: materials });
  } catch (error) {
    return res.status(500).json({ error: "Failed to get all materials." });
  }
});
